...
| 평가 | 실제 집필 문제에서 도출한 7개 유형·70건 합성 회귀셋, 3단계 판정, 유형별 실패 분석 |
...
현재 `rule-v1` 기준선은 70건 중 14건(20%)을 통과합니다. 직접 규칙은 10/10을 통과하지만 간접 표현, 정상적인 설정 변화, 회상·거짓말·불확실한 기억 같은 문맥을 처리하지 못합니다. 낮은 점수를 숨기지 않고 다음 개선의 출발점으로 기록했으며 상세 결과는 [평가 보고서](docs/EVALUATION.md)에서 확인할 수 있습니다.
...
src/data/evaluation.ts  7개 유형·70건 공개 합성 평가셋
